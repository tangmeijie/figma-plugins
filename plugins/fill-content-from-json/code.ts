...
function findAllTextNodesInNode(node: SceneNode): TextNode[] {
  ...
  if (node.type === 'TEXT') {
    ...
    // If the node has children (is a container like a Frame or Group)
...
/**
 * Recursively find all text nodes with their full path from root node
 */
function findAllTextNodesWithPath(node: SceneNode, path: SceneNode[] = []): Array<{node: TextNode, path: SceneNode[]}> {
  const results: Array<{node: TextNode, path: SceneNode[]}> = [];
  const currentPath = [...path, node];
  
  if (node.type === 'TEXT') {
    results.push({node: node as TextNode, path: currentPath});
  } else if ('children' in node) {
    // If the node has children (is a container like a Frame or Group)
    for (const child of node.children) {
